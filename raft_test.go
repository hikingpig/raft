...
func TestElectionLeaderDisconnectThenReconnect3(t *testing.T) {
	h := NewHarness(t, 3)
	defer h.Shutdown()
	origLeaderId, _ := h.CheckSingleLeader()
	h.DisconnectPeer(origLeaderId)
...
func TestNoCommitWithNoQuorum(t *testing.T) {
	...
	// A new leader will be elected. It will be a different node because the current leader
	// must step down as its term is behind
	// The two disconnected node will elect the other.
